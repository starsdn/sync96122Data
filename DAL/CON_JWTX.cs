...
namespace DAL
{
	...
	public partial class CON_JWTX
	{
		...
		public bool Exists(string ID)
		{
			...
			strSql.Append("select count(1) from CON_JWTX");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into CON_JWTX(");
			strSql.Append("ID,BH,SJBH,FYR,LXDH,DZ,DZYJ,FYRQ,BLQX,BT,FYNR,NRZY,YQ,FJDZ,FPSJ,JBRQ,JBYQ,LDPS,BZ,CJR,CJSJ,ZT,FJM,XJLY)");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update CON_JWTX set ");
			strSql.Append("BH=:BH,");
		...
		public bool Delete(string ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from CON_JWTX ");
			strSql.Append(" where ID=:ID ");
			OracleParameter[] parameters = {
					new OracleParameter(":ID", OracleDbType.Varchar2,36)			};
			parameters[0].Value = ID;
		...
		public Model.CON_JWTX GetModel(string ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,BH,SJBH,FYR,LXDH,DZ,DZYJ,FYRQ,BLQX,BT,FYNR,NRZY,YQ,FJDZ,FPSJ,JBRQ,JBYQ,LDPS,BZ,CJR,CJSJ,ZT,FJM,XJLY from CON_JWTX ");
			strSql.Append(" where ID=:ID ");
			OracleParameter[] parameters = {
					new OracleParameter(":ID", OracleDbType.Varchar2,36)			};
			parameters[0].Value = ID;

			Model.CON_JWTX model=new Model.CON_JWTX();
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.CON_JWTX DataRowToModel(DataRow row)
		{
			Model.CON_JWTX model=new Model.CON_JWTX();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,BH,SJBH,FYR,LXDH,DZ,DZYJ,FYRQ,BLQX,BT,FYNR,NRZY,YQ,FJDZ,FPSJ,JBRQ,JBYQ,LDPS,BZ,CJR,CJSJ,ZT,FJM,XJLY ");
			strSql.Append(" FROM CON_JWTX ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from CON_JWTX T ");
