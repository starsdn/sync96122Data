...
namespace DAL
{
	...
	/// 数据访问类:CON_DCTXLX
	/// </summary>
	public partial class CON_DCTXLX
	{
		public CON_DCTXLX()
		{}
		#region  BasicMethod
		...
		public bool Exists(string ID)
		{
			...
			strSql.Append("select count(1) from CON_DCTXLX");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(Model.CON_DCTXLX model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into CON_DCTXLX(");
			strSql.Append("ID,BH,SJBH,FYR,LXDH,DZYJ,DZ,FYRQ,BLQX,BT,FYNR,NRZY,YQ,FJDZ,FPSJ,JBRQ,JBYQ,FYWTXZ,LDPS,BZ,CJR,CJSJ,ZT,FJM,XJLY)");
			...
			OracleParameter[] parameters = {
					...
					new OracleParameter(":FYWTXZ", OracleDbType.Varchar2,50),
			...
			parameters[17].Value = model.FYWTXZ;
			...
			parameters[24].Value = model.XJLY;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Model.CON_DCTXLX model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update CON_DCTXLX set ");
			strSql.Append("BH=:BH,");
			...
			parameters[16].Value = model.FYWTXZ;
			...
			parameters[23].Value = model.XJLY;
		...
		public bool Delete(string ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from CON_DCTXLX ");
			strSql.Append(" where ID=:ID ");
			OracleParameter[] parameters = {
					new OracleParameter(":ID", OracleDbType.Varchar2,36)			};
			parameters[0].Value = ID;
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.CON_DCTXLX GetModel(string ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,BH,SJBH,FYR,LXDH,DZYJ,DZ,FYRQ,BLQX,BT,FYNR,NRZY,YQ,FJDZ,FPSJ,JBRQ,JBYQ,FYWTXZ,LDPS,BZ,CJR,CJSJ,ZT,FJM,XJLY from CON_DCTXLX ");
			strSql.Append(" where ID=:ID ");
			OracleParameter[] parameters = {
					new OracleParameter(":ID", OracleDbType.Varchar2,36)			};
			parameters[0].Value = ID;

			Model.CON_DCTXLX model=new Model.CON_DCTXLX();
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.CON_DCTXLX DataRowToModel(DataRow row)
		{
			Model.CON_DCTXLX model=new Model.CON_DCTXLX();
			if (row != null)
			{
				...
				if(row["FYWTXZ"]!=null)
				{
					model.FYWTXZ=row["FYWTXZ"].ToString();
				}
				if(row["LDPS"]!=null)
				{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,BH,SJBH,FYR,LXDH,DZYJ,DZ,FYRQ,BLQX,BT,FYNR,NRZY,YQ,FJDZ,FPSJ,JBRQ,JBYQ,FYWTXZ,LDPS,BZ,CJR,CJSJ,ZT,FJM,XJLY ");
			strSql.Append(" FROM CON_DCTXLX ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from CON_DCTXLX T ");
