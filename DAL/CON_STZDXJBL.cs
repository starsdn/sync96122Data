...
namespace DAL
{
	...
	/// 数据访问类:CON_STZDXJBL
	/// </summary>
	public partial class CON_STZDXJBL
	{
		public CON_STZDXJBL()
		{}
		#region  BasicMethod
		...
		public bool Exists(string ID)
		{
			...
			strSql.Append("select count(1) from CON_STZDXJBL");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(Model.CON_STZDXJBL model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into CON_STZDXJBL(");
			strSql.Append("ID,XJLY,BH,SJBH,LXR,LXDH,DZ,DZYJ,FYRQ,BLQX,BT,FYNR,NRZY,YQ,FJDZ,FPSJ,JBRQ,JBYQ,LDPS,BZ,CJR,CJSJ,ZT,FJM)");
			...
			OracleParameter[] parameters = {
					...
					new OracleParameter(":LXR", OracleDbType.Varchar2,100),
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Model.CON_STZDXJBL model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update CON_STZDXJBL set ");
		...
		public bool Delete(string ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from CON_STZDXJBL ");
			strSql.Append(" where ID=:ID ");
			OracleParameter[] parameters = {
					new OracleParameter(":ID", OracleDbType.Varchar2,36)			};
			parameters[0].Value = ID;
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.CON_STZDXJBL GetModel(string ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,XJLY,BH,SJBH,LXR,LXDH,DZ,DZYJ,FYRQ,BLQX,BT,FYNR,NRZY,YQ,FJDZ,FPSJ,JBRQ,JBYQ,LDPS,BZ,CJR,CJSJ,ZT,FJM from CON_STZDXJBL ");
			strSql.Append(" where ID=:ID ");
			OracleParameter[] parameters = {
					new OracleParameter(":ID", OracleDbType.Varchar2,36)			};
			parameters[0].Value = ID;

			Model.CON_STZDXJBL model=new Model.CON_STZDXJBL();
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.CON_STZDXJBL DataRowToModel(DataRow row)
		{
			Model.CON_STZDXJBL model=new Model.CON_STZDXJBL();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,XJLY,BH,SJBH,LXR,LXDH,DZ,DZYJ,FYRQ,BLQX,BT,FYNR,NRZY,YQ,FJDZ,FPSJ,JBRQ,JBYQ,LDPS,BZ,CJR,CJSJ,ZT,FJM ");
			strSql.Append(" FROM CON_STZDXJBL ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from CON_STZDXJBL T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "CON_STZDXJBL";
